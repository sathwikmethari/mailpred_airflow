...
async def async_get_batched_main(id_chunks: list[list[str]], token_path: str, coro_num: int) -> dict:
    """Importing Functions."""
    from utils.gm_main_utils import worker, generate_services, async_get_payload

    services = generate_services(coro_num, token_path)
    partial_functions = [partial(async_get_batched_payload, service) for service in services]
    ...
    tasks = [asyncio.create_task(worker_for_batched(id, function, in_queue, out_queue, fail_queue)) for id, function in enumerate(partial_functions, start=1)]
